...
struct MenuItems:Decodable {
    ...
    static func downloadData(feedUrl:String, completion: @escaping ([Menu], Branding) -> Void) {
        ...
        if let urlObject = url {
            URLSession.shared.dataTask(with: urlObject) { (data, ressponse, error) in
                ...
                do {
                    ...
                    let branding = feed.branding
                    let menu = feed.menuItems
                    
                    DispatchQueue.main.async {
                        guard let dataBranding = branding else { return }
                        guard let dataMenu = menu else { return }
                        
                        completion(dataMenu, dataBranding)
                    }
